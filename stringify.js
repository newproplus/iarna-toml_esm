...
function toJSON(obj) {
  const nobj = Array.isArray(obj) ? [] : Object.prototype.hasOwnProperty.call(obj, '__proto__') ? { ['__proto__']: undefined } : {}
...
function stringifyObject(prefix, indent, obj) {
  obj = toJSON(obj)
  const inlineKeys = getInlineKeys(obj)
  const complexKeys = getComplexKeys(obj)
  const result = []
  const inlineIndent = indent || ''
  inlineKeys.forEach(key => {
    const type = tomlType(obj[key])
